...
class PurchaseOrder(models.Model):
    ...
    #                 # For each product in the purchase order lines
    #                 # Check if the product is in the sale order lines
    #                 if product in existing_products_in_sale_order:
    #                     count -= 1  # Subtract 1 for the existing occurrence in sale order lines
    #
    #                 if count > 0:
    #                     # Add lines for the remaining count
    #                     order_line_vals = {
        ...
        if revised_prices:
            ...
            if sale_order:
                ...
                if salesperson and salesperson.email:
                    try:
                        mail_template = self.env.ref("amc_management.email_template_revised_prices")
...
class PurchaseOrderProductSummary(models.Model):
    ...
    sales_order_id = fields.Many2one('sale.order', string='Purchase Order', required=True, ondelete='cascade')
...
class ProductCostLog(models.Model):
    ...
    _description = 'Product Cost Log'

    product_id = fields.Many2one('product.product', string='Product', required=True)
    original_cost = fields.Float('Original Cost', required=True)
    revised_cost = fields.Float('Revised Cost', required=True)
    log_date = fields.Datetime('Log Date', required=True, default=lambda self: fields.Datetime.now())
    ...
    def create(self, vals):
        ...
        vals['original_cost'] = product.standard_price
        return super(ProductCostLog, self).create(vals)
...
class SaleOrder(models.Model):
    ...
    product_cost_log_ids = fields.One2many('product.cost.log', 'sale_order_id', string='Product Cost Logs')
...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals):
        ...
        res._update_product_cost_log()
        return res

    def write(self, vals):
        res = super(SaleOrderLine, self).write(vals)
        self._update_product_cost_log()
        return res

    def _update_product_cost_log(self):
        for line in self:
            sale_order = line.order_id
            product_cost_log_vals = {
                'product_id': line.product_id.id,
                'revised_cost': line.purchase_price,
                'sale_order_id': sale_order.id,
            }
            product_cost_log = self.env['product.cost.log'].create(product_cost_log_vals)
