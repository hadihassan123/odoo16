...
class AmcDashboard(models.Model):
    _name = 'amc.dashboard'
    _description = "AMC and Maintenance Dashboard"

    name = fields.Char(string="Name", default="AMC & Maintenance Overview")
    amc_count = fields.Integer(string="Number of AMCs", compute="_compute_amc_count")
    open_ticket_count = fields.Integer(string="Open Tickets", compute="_compute_open_ticket_count")
    overdue_ticket_count = fields.Integer(string="Overdue Tickets", compute="_compute_overdue_ticket_count")
    amc_graph = fields.Binary(string="AMC Graph Data")  # Optional for chart
    ticket_graph = fields.Binary(string="Ticket Graph Data")  # Optional for chart




    def _compute_amc_count(self):
        amc_count = self.env['amc.customers']
        for record in self:
            if record.asset_upload_id:
                record.amc_count = len(record.asset_upload_id.asset_upload_ids)
            else:
                record.amc_count = 0
    ...
    def _compute_ticket_count(self):
        ticket_count = self.env['maintenance.customers'].search_count([])
        self.ticket_count = ticket_count

    def _compute_open_ticket_count(self):
        open_tickets = self.env['maintenance.customers'].search_count([('state', '=', 'open')])
        self.open_ticket_count = open_tickets

    def _compute_overdue_ticket_count(self):
        today = fields.Date.today()
        overdue_tickets = self.env['maintenance.customers'].search_count([
            ('state', '=', 'assigned'),
            # ('due_date', '<', today)
        ])
        self.overdue_ticket_count = overdue_tickets
